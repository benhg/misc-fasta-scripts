...
if len(sys.argv) != 2:
    print("Usage: python3 {} <input_file>".format(__file__))
    exit(1)
...
for motif in motif_list:
    file = open("{}.csv".format(motif['name']), 'w')
    writer = csv.writer(file)
    writer.writerow(["sequence_id", "sequence_contents"])
    regex = motif['regex']
    for i, line in enumerate(in_text):
        if '>' in line:
            if bool(re.match(regex, in_text[i+1])):
                ...
                writer.writerow([seq_id, seq_contents])
    file.close()
